...
# Gemini API Setup
GOOGLE_API_KEY = os.getenv("GOOGLE_API_KEY", "AIzaSyDDaUTEWZGkEvfT46SVH_qOs_QPQJcHLsg")
try:
...
async def generate_recipe(user_input: str, feedback: str = None):
    ...
    if terminate_flag:
        return {"content": "TERMINATE", "name": "RecipeCreator"}
    ...
    try:
        ...
        prompt = (
            ...
            f"and 'instructions' (list of 3-5 concise strings)."
        )
        ...
        try:
            ...
            recipe_data = {
                ...
                "servings": servings,
                "ingredients": [
                    {"item": ing["name"], "amount": f"{ing['quantity_g']}g"}
                    for ing in recipe_ingredients
                ],
                "instructions": [
...
def check_nutrition(recipe, user_input):
    ...
    try:
        ...
        if "content" in recipe and recipe["content"] == "TERMINATE":
            terminate_flag = True
            return "TERMINATE"
        recipe_data = recipe
        if not recipe_data:
        ...
        for ing in recipe_data["ingredients"]:
            ...
            if not db_ing:
                return f"Ingredient {ing['item']} not found in database. Use only ingredients from database."
        ...
        },
        {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "user_input": {"type": "string", "description": "User preferences"}
                },
...
# Custom AssistantAgent with termination check
class TerminatingAssistantAgent(AssistantAgent):
    async def a_generate_reply(self, messages=None, sender=None, **kwargs):
        global terminate_flag
        if terminate_flag or os.path.exists("terminate.flag"):
            return {"content": "TERMINATE", "name": self.name}
        # Check last message for TERMINATE
        if messages:
            last_message = messages[-1]
            if (isinstance(last_message, dict) and "content" in last_message and
                "TERMINATE" in last_message.get("content", "").upper() and
                last_message.get("name") == "NutritionChecker"):
                return {"content": "TERMINATE", "name": self.name}
        reply = await super().a_generate_reply(messages=messages, sender=sender, **kwargs)
        # Log reply for debugging
        print(f"DEBUG: {self.name} reply: {reply}")
        return reply
...
class CustomGroupChatManager(GroupChatManager):
    async def a_run_chat(self, messages=None, sender=None, config=None):
        global terminate_flag
        if terminate_flag or os.path.exists("terminate.flag"):
            for msg in reversed(self._groupchat.messages):
                if isinstance(msg, dict) and "content" in msg:
                    return msg
            return {"content": "No messages", "name": "Manager"}
        ...
        while round_count < groupchat.max_round:
            ...
            # Skip None or invalid replies
            if reply is None or not (isinstance(reply, dict) and "content" in reply):
                print(f"DEBUG: Skipping invalid reply from {speaker.name}: {reply}")
                break
...
recipe_creator = TerminatingAssistantAgent(
    name="RecipeCreator",
    llm_config=function_config,
    system_message=(
        "You are a chef. Call generate_recipe with user_input and feedback (if provided) to create a recipe JSON. "
        "Output ONLY the recipe JSON. Do NOT respond if the previous message from NutritionChecker contains 'TERMINATE'."
    ),
    function_map={"generate_recipe": generate_recipe}
)

nutrition_checker = TerminatingAssistantAgent(
    ...
    system_message=(
        ...
        "Do NOT respond after outputting 'TERMINATE'."
    ),
...
async def run_chef(preferences):
    global terminate_flag
    terminate_flag = False  # Reset flag
    ...
    try:
        ...
        #print(f"❌ TypeError in chat: {e}. Check reply formats.")
        print("TERMINATING.")
    except Exception as e:
