...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('faq4.log')  # Log to file only
    ]
)
...
class QueryHandlerAgent(ConversableAgent):
    ...
    async def a_generate_reply(self, messages=None, sender=None, config=None):
        try:
            ...
            prompt = f"""You are a query handler for an FAQ chatbot. Validate and rephrase the user's query to make it clear, concise, and aligned with common FAQ phrasing in a document. Preserve key terms (e.g., '2FA', 'set up') and include synonyms (e.g., 'enable', 'configure', 'two-factor authentication') to improve retrieval. Return only the rephrased query.

User Query: {latest_query}

Instructions:
- If clear, return unchanged or slightly refined.
- If vague, rephrase for specificity, keeping original terms and adding synonyms (e.g., 'set up 2FA' -> 'enable two-factor authentication (2FA)').
- If invalid (e.g., not a question, offensive), return "INVALID: Please ask a valid FAQ-related question."
- Do not answer the query; only rephrase.
- Example: 'how to set up 2FA?' -> 'How to enable two-factor authentication (2FA)?'
"""
...
async def main():
    ...
    while True:
        ...
        if question.lower() == "exit":
            ...
            logger.info("FAQ pipeline completed")
            break
