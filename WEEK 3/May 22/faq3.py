...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('faq3.log')  # Log to file only
    ]
)
...
class QueryHandlerAgent(ConversableAgent):
    ...
    async def a_generate_reply(self, messages=None, sender=None, config=None):
        try:
            ...
            prompt = f"""You are a query handler for an FAQ chatbot. Validate and, if necessary, rephrase the user's query to make it clear and concise for retrieval. Return only the validated or rephrased query.

User Query: {latest_query}

Instructions:
- If the query is clear, return it unchanged.
- If vague or ambiguous, rephrase it to be specific and concise.
- If invalid (e.g., not a question, offensive), return "INVALID: Please ask a valid FAQ-related question."
- Do not answer the query; only validate or rephrase.
"""
